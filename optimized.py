...
class AlgoDynamique:
    ...
    def results(self, matrice):
        ...
        while w >= 0 and n >= 0:  # tant que la capacité est >= à 0 et que nous
            # n'avons pas parcourus tout les éléments
            element = data[n - 1]
            if matrice[n][int(w)] == matrice[int(n - 1)][
                int(w - element[1])] + self.profit(element):
        ...
        total_cost = 0
        total_profit = 0
        nb_letter_name = len(row_data[0][0])  # we do this because we have
        # multiplied all data per 100
        name_actions = []
    ...
    def algo(self):
        ...
        matrice = [[0 for x in range(int((self.capacity + 1)))] for x in
                   range(len(data) + 1)]
        ...
        for i in range(1, lines):  # ligne
            for x in range(1, colomns):  # colonne
                if data[i - 1][
                    1] <= x:  # si le prix de l'élément courant est inférieur
                    # à la capacité
                    matrice[i][x] = max(matrice[i - 1][x],
                                        # valeur de la ligne précédante
                                        matrice[i - 1][int(x - data[i - 1][
                                            1])] +
                                        # matrice[i-1][capacité - poids de
                                        # l'élément courant => place qui reste
                                        # dispo]
                                        self.profit(data[i - 1])) # valeur de
                    # mon element courant
                else:
                    matrice[i][x] = matrice[i - 1][
                        x]  # on garde la valeur de i-1
...
file_1 = 'dataset1_Python+P7.csv'
file_2 = 'dataset2_Python+P7.csv'
dt = pd.read_csv(file_2)
df = pd.DataFrame(dt)
